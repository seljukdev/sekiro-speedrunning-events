...
const FIRST_MATCHES = [
    { p1: CONTESTANTS[11], p2: CONTESTANTS[12], type: "Elimination R1", winner: "ZenoZilch", matchLink: "https://www.youtube.com/live/DbKLyeGvkdY?si=b4qApfUAvminFW0d&t=954" },
    { p1: CONTESTANTS[13], p2: CONTESTANTS[14], type: "Elimination R1", winner: "GamingMusume", matchLink: "https://www.youtube.com/live/c1e-cULX3HQ" },
    { p1: CONTESTANTS[15], p2: CONTESTANTS[16], type: "Elimination R1", winner: "Limmitd", matchLink: "https://www.youtube.com/live/J86_IBXVN00?si=MC5VdOM7NIPHR2lx&t=13618" },
    { p1: CONTESTANTS[17], p2: CONTESTANTS[18], type: "Elimination R1", winner: "banjobunny", matchLink: "https://www.youtube.com/live/J86_IBXVN00?si=hDfHjvfXeLwrRigA&t=8673" },
    { p1: CONTESTANTS[19], p2: CONTESTANTS[20], type: "Elimination R1", winner: "papa_pp69", matchLink: "https://www.youtube.com/live/J86_IBXVN00?si=CyMsX3EVpMPMUIVe&t=17396" },
];

const SECOND_MATCHES = [
    { p1: CONTESTANTS[0], p2: CONTESTANTS[20], type: "Winners R2", winner: "WormdogBS", status: "Finished", matchLink: "https://www.youtube.com/live/rfjVXP9S1TY?si=HO62oChZ43I0SAvE&t=3696" },
    { p1: CONTESTANTS[1], p2: CONTESTANTS[17], type: "Winners R2", winner: "albertitotf", matchLink: "https://www.youtube.com/live/u73CbxgfP6c?si=kksrUbfJMIEH5dhK&t=19294" },
    { p1: CONTESTANTS[2], p2: CONTESTANTS[7], type: "Winners R2", winner: "ramonchi_5", status: "Finished", matchLink: "https://www.youtube.com/live/yz2h0cC2LsU?si=AUVv0JYcE0RuKqFC&t=8211" },
    { p1: CONTESTANTS[3], p2: CONTESTANTS[14], type: "Winners R2", winner: "mommyemma77", status: "Walkover" },
    { p1: CONTESTANTS[4], p2: CONTESTANTS[11], type: "Winners R2", winner: "Holm_GG", status: "Finished", matchLink: "https://www.youtube.com/live/rfjVXP9S1TY?si=djAJ2aieaX0NmJ15&t=1115" },
    { p1: CONTESTANTS[5], p2: CONTESTANTS[10], type: "Winners R2", winner: "ymir_happy", matchLink: "https://www.youtube.com/live/G3nxvPQn6rE?si=bYa_HkpnBidwC5Zy&t=4194" },
    { p1: CONTESTANTS[9], p2: CONTESTANTS[6], type: "Winners R2", winner: "gunjou1213", matchLink: "https://www.youtube.com/live/G3nxvPQn6rE?si=W6Tub0my2Fvy0-nz&t=361" },
];
...
const UPPER_MATCHES_R3 = [
    { p1: CONTESTANTS[0], p2: CONTESTANTS[3], type: "Winners R3: QF", status: "Wormdog vs Emma" },
    { p1: CONTESTANTS[1], p2: CONTESTANTS[2], type: "Winners R3: QF", status: "Albert vs Ramon" },
    { p1: CONTESTANTS[9], p2: CONTESTANTS[5], type: "Winners R3: QF", status: "Gunjou vs Ymir" },
    { p1: CONTESTANTS[4], p2: { name: "BYE" }, type: "Winners R3: Bye", winner: "Holm_GG" },
];

const LOWER_MATCHES_R3 = [
    { p1: CONTESTANTS[13], p2: CONTESTANTS[10], type: "Lower R3", status: "Pennek vs Zerowww7" },
    { p1: CONTESTANTS[12], p2: CONTESTANTS[17], type: "Lower R3", status: "Krrag2 vs Banjobunny" },
    // sarah vs leech
    { p1: CONTESTANTS[7], p2: CONTESTANTS[19], type: "Lower R3", status: "Sara vs Leech" },
    // { p1: CONTESTANTS[7], p2: { name: "TBD" }, type: "Lower R3", status: "Sara vs TBD" },
    { p1: CONTESTANTS[11], p2: CONTESTANTS[20], type: "Lower R3", status: "Zeno vs Papa" },
];

// --- COMPONENTS ---

const AnimatedUnderline = () => (
    <motion.svg className="absolute -bottom-2 md:-bottom-4 left-0 w-full h-[12px] md:h-[20px] z-0 pointer-events-none" viewBox="0 0 100 10" preserveAspectRatio="none">
        <motion.path d="M 0 5 Q 50 10, 100 5" fill="transparent" strokeWidth="2" stroke="currentColor" strokeLinecap="round" initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 0.8 }} transition={{ delay: 1.5, duration: 0.8, ease: "easeOut" }} className="text-red-600" style={{ filter: "drop-shadow(0px 0px 8px rgba(220, 38, 38, 0.8))" }} />
    </motion.svg>
);

const SakuraBackground = memo(() => {
    const [petals] = useState(() => Array.from({ length: 15 }).map((_, i) => ({ id: i, left: Math.random() * 100 + "%", delay: Math.random() * 10, duration: 15 + Math.random() * 20 })));
    return (
        <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
            {petals.map((p) => (
                <motion.div key={p.id} initial={{ y: -20, opacity: 0 }} animate={{ y: ["-5vh", "110vh"], opacity: [0, 0.5, 0], x: [0, 30, -30, 0] }} transition={{ duration: p.duration, repeat: Infinity, delay: p.delay, ease: "linear" }} className="absolute w-2 h-2 bg-red-500/50 rounded-full blur-[1px]" style={{ left: p.left }} />
            ))}
        </div>
    );
});

const SectionTitle = memo(({ title, subtitle, colorClass = "text-stone-100" }) => (
    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-12">
        <div className="relative inline-block">
            <h3 className={`text-4xl md:text-6xl font-['Cinzel_Decorative'] ${colorClass} font-bold uppercase tracking-[0.15em] relative z-10`}>
                {title}
            </h3>
            <motion.div initial={{ width: 0 }} whileInView={{ width: "100%" }} className="h-[2px] bg-gradient-to-r from-transparent via-red-700 to-transparent mt-2 mx-auto shadow-[0_0_10px_rgba(185,28,28,0.8)]" />
        </div>
        {subtitle && <p className="text-red-600 mt-4 font-['Cinzel'] font-bold tracking-[0.4em] uppercase text-xs opacity-80">{subtitle}</p>}
    </motion.div>
));

const PlayerRow = memo(({ player, isWinner, isLoser, isBye }) => (
    <div className={`flex items-center justify-between py-1 transition-all duration-500 ${isLoser ? "opacity-40 grayscale" : "opacity-100"}`}>
        <div className="flex items-center gap-3 relative group/player">
            <div className="w-5 flex justify-center">
                {isWinner && <Trophy size={16} className="text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.6)]" />}
                {isBye && <Crown size={16} className="text-yellow-600" />}
            </div>
            <div className="relative">
                <a href={`https://twitch.tv/${player.name}`} target="_blank" rel="noopener noreferrer" className={`text-xl font-['Cinzel'] font-black tracking-tight transition-colors hover:text-red-500 ${isWinner || isBye ? "text-white" : "text-stone-300"}`}>
                    {player.name}
                </a>
                {isLoser && <motion.div initial={{ width: 0 }} animate={{ width: "110%" }} className="absolute top-1/2 -left-[5%] h-[2px] bg-red-700 shadow-[0_0_5px_rgba(185,28,28,0.8)]" />}
            </div>
        </div>
    </div>
));

const MatchCard = memo(({ p1, p2, type, winner = null, matchLink = null, isLive = false, status = null }) => {
    ...
    return (
        <motion.div variants={FADE_IN_VARIANTS} whileHover={{ scale: 1.01, borderColor: "rgba(220,38,38,0.4)" }} className={`relative bg-[#080808] border border-stone-800/60 p-4 transition-all duration-300 shadow-2xl overflow-hidden ${isLive ? "border-red-600 ring-1 ring-red-600/20 shadow-red-900/10" : ""}`}>
            ...
            <div className="flex flex-col gap-1">
                <PlayerRow player={p1} isWinner={isP1Winner} isLoser={isFinished && !isP1Winner} isBye={isBye} />
                {!isBye && p2 && (
                    <>
                        <div className="flex items-center gap-4 my-1 opacity-20">
                            ...
                            <span className="text-[10px] font-black italic font-['Cinzel']">VS</span>
                            <div className="h-[1px] flex-1 bg-gradient-to-l from-transparent to-stone-500" />
                        </div>
                        <PlayerRow player={p2} isWinner={isP2Winner} isLoser={isFinished && !isP2Winner} isBye={false} />
                    </>
                )}
            </div>
            {matchLink && !isBye && (
                <div className="mt-4 pt-4 border-t border-stone-800/40">
                    <a href={matchLink} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full text-[10px] font-['Cinzel'] font-bold tracking-[0.2em] uppercase py-2 bg-stone-900 border border-stone-800 hover:bg-red-700 hover:border-red-600 transition-all duration-300">
                        {isLive ? <Tv size={14} /> : <Play size={14} />} {isLive ? "Enter Stream" : "Watch Full VOD"}
                    </a>
                </div>
            )}
        </motion.div>
...
export default function App() {
    return (
        <div className="min-h-screen bg-[#050505] text-stone-300 font-['EB_Garamond'] text-lg selection:bg-red-900 overflow-x-hidden">
            ...
            {/* BRACKETS */}
            <section id="brackets" className="py-24 px-4 max-w-7xl mx-auto space-y-32">
                
                {/* ROUND 3: UPPER */}
                <div>
                    <SectionTitle title="The Conflict" subtitle="Winners Round 3: Quarter-Finals" />
                ...
                {/* ROUND 3: LOWER */}
                <div>
                    <SectionTitle title="The Abyss" subtitle="Lower Bracket Round 3: Survival" colorClass="text-orange-100" />
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className=" border-t border-stone-900/50">
                    <h4 className="text-center font-['Cinzel'] text-stone-600 tracking-[0.5em] uppercase text-sm mb-12">Historical Record: Round 2</h4>
                ...
                <div className=" border-t border-stone-900/50">
                    <h4 className="text-center font-['Cinzel'] text-stone-600 tracking-[0.5em] uppercase text-sm mb-12">Historical Record: Round 2 Lower</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 opacity-60 hover:opacity-100 transition-opacity">
                ...
                <div className=" border-t border-stone-900/50">
                    <h4 className="text-center font-['Cinzel'] text-stone-600 tracking-[0.5em] uppercase text-sm mb-12">Historical Record: Round 1</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 opacity-60 hover:opacity-100 transition-opacity">
